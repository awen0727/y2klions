...
6. 回到 Google Sheet，確認出現「千禧獅子會系統」選單、`SetupGuide` 工作表，以及 `Members` 內的預設資料：

```text
M000 / 王立文 / 0932368727 / 系統管理員
```

這筆資料保留給系統管理用途，不計入一般會員總數。
...
`M000` 是預設系統管理員資料，請勿放入一般會員人數統計。若既有資料庫尚未建立這筆資料，可在 Apps Script 手動執行：
